...
class BUPTUser(BaseModel):
    ...
    def ncov_checkin(self, force=False):
        ...
        if self.cookie_eaisess != None:
            cookies = {"eai-sess": self.cookie_eaisess, "UUKey": self.cookie_uukey}
            requests.utils.add_dict_to_cookiejar(session.cookies, cookies)

        report_page_resp = session.get(
            REPORT_PAGE, allow_redirects=False, timeout=API_TIMEOUT
        )
    ...
    def xisu_ncov_checkin(self, force=False):
        ...
        history_data = session.get(
            XISU_HISTORY_DATA, allow_redirects=False, timeout=API_TIMEOUT
        )
        ...
        assert (
            "realname" in ncov_report_page_html
        ), f"Xisu Report 中的报告页面 {REPORT_PAGE} 返回信息不正确"

        post_data = build_xisu_ncov_checkin_post_data(
            ncov_report_page_html=ncov_report_page_html,
            xisu_nconv_checkin_pending_form=xisu_nconv_checkin_pending_form,
        )
        ...
        xisu_report_api_resp = session.post(
            XISU_REPORT_API,
            post_data,
            headers={
                "Accept": "application/json",
